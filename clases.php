...
class conectar {
    public function conecta($host, $user, $pass, $db) {
        if (mysql_connect($host, $user, $pass)) {
            //echo "Ha establecido conexion";
            if (mysql_select_db($db)) {
                //echo " y se ha seleccionado correctamente la base de datos";
                
            } else {
                echo " No existe la base de datos que usted definio";
            }
        } else {
...
$conectar = new conectar(); //CONECTAMOS A LA DB
$conectar->conecta("localhost", "user", "password", "db"); //INGRESAMOS LOS DATOS
class usuario {
...
class modelos {
    ...
    public function registro($usuario, $password) {
        ...
        $usuario = addslashes(trim($usuario));
        $pass_cript = sha1(addslashes($password)); //encriptamos la password
        ...
        if ($localizo > 0) {
            ...
            if ($sql) {
                ...
                echo "Hubo un problema al procesar el registro"; //. mysql_error();
            }
        }
    ...
    public function historial_entradas($ingreso) {
        ...
        if ($sql) {
            ...
            echo "Hubo un problema al actualizar el historial de entradas: "; //. mysql_error();
        }
    }
    public function historial_salidas($salida, $causa) {
        ...
        if ($sql) {
            ...
            echo "Hubo un problema al actualizar el historial de salidas: "; //. mysql_error();
        }
    }
    public function add_money($plata) {
        ...
        if (mysql_num_rows($sql) > 0) {
            ...
            if ($sql) {
                ...
                echo "Hubo un problema al guardar tu ingreso"; //. mysql_error();
            }
        } else {
	...
	if($dinero_viejo < $se_saca){
		echo "No puedes sacar dinero que no tienes";		exit();
		}else{
            ...
            if ($sql) {
                ...
                echo "Hubo un problema al sacar dinero ";// . mysql_error();
            }
}
        } else {
